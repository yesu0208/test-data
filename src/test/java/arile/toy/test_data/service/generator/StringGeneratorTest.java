...
class StringGeneratorTest {
    ...
    void givenParams_whenGenerating_thenReturnsRandomText() throws JsonProcessingException {
        ...
        assertThat(result).containsPattern("[가-힣]{1,10}$"); // 정규표현식
    ...
    """)
    @DisplayName("옵션에 따라 정규식을 통과하는 랜덤 문자열을 생성한다.")
    @ParameterizedTest(name = "{index}. {0} ===> 통과 정규식: {1}") // test method에 인자를 넣을 수 있다. (위의 CsvSource로부터)
    ...
    """) // 최솟값은 1부터, max가 min보다 작으면 x, 최댓값은 10까지 : boundary test
    ...
    void givenWrongOption_whenGenerating_thenThrowsException(String optionJson) throws Exception {
        ...
        assertThat(t)
                ...
                .hasMessageStartingWith("[가짜 데이터 생성 옵션 오류]"); // 이 메시지로 시작하는 오류
        then(mapper).should().readValue(optionJson, StringGenerator.Option.class);
        then(mockDataRepository).shouldHaveNoInteractions(); // 오류가 났으니, repository에는 가지 않았으면 좋겠다.
    }
    ...
    void givenNoParams_whenGenerating_thenReturnsRandomText() {
        ...
        then(mapper).shouldHaveNoInteractions(); // option이 없으므로 mapper는 동작 x
        then(mockDataRepository).should().findByMockDataType(mockDataType);
    }
    ...
    void givenNoParams_whenGenerating_thenReturnsNull() {
        ...
        then(mapper).shouldHaveNoInteractions(); // null return 시 필요 x
        then(mockDataRepository).shouldHaveNoInteractions(); // null return 시 필요 x
    }
    ...
    @DisplayName("강제 값이 주어지더라도, blank 옵션에 따라 null을 반환할 수 있다.") // blank 옵션이 우선
    @Test
